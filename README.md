...
cp local.gekyume.powermodemenu.plist "$HOME/Library/LaunchAgents/"
launchctl bootout "gui/$(id -u)/local.gekyume.powermodemenu" 2>/dev/null || true
launchctl bootstrap "gui/$(id -u)" "$HOME/Library/LaunchAgents/local.gekyume.powermodemenu.plist"
```
...
```bash
launchctl print "gui/$(id -u)/local.gekyume.powermodemenu"
pmset -g
pmset -g assertions
```
...
- `local.gekyume.powermodemenu.plist`：用户级 LaunchAgent
